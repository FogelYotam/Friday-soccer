...
<div id="tab-kosher" class="tab">
  <details class="card" style="margin-bottom:12px">
    ...
    <div style="font-size:.82rem;color:#cbd5e1;line-height:1.7;margin-top:10px">
      ...
      <b style="color:#10b981">התקפה</b> — שערים ובישולים <b>ביחס לעמדה שלך</b>: כל אחד נמדד מול הממוצע והפיזור של העמדה שלו, לא מול חלוצים. שוערים ומגנים מבקיעים הרבה פחות, ולכן כל שער שלהם קופץ הרבה יותר — שער+בישול למשחק שווה יותר לשוער, אחריו מגן ואז חלוץ. שער נספר כפול 1.7 מבישול.<br>
...
const ATT_POS_MULT = {'שוער':1.2, 'הגנה':0.8, 'התקפה':1.0};
const ATT_SCALE    = 150;
...
function buildKosher() {
  document.getElementById('kosherFormula').textContent =
    `דירוג כוח = בסיס (רמת הקריירה, כולל חוזק היריבים) + התקפה (שערים ובישולים מול הממוצע והפיזור של אותה עמדה — שער+בישול למשחק שווה שער שווה פי 1.7 מבישול) + הגנה (כמה ספגת מול מה שהרכב הקבוצה חזה) + כושר 6 חודשים − היעדרות. מוצגים כל השחקנים עם מעל ${KSR_MIN_CAREER_GAMES} משחקים.`;
