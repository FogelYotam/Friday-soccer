...
<div id="tab-kosher" class="tab">
  <details class="card" style="margin-bottom:12px">
    ...
    <div style="font-size:.82rem;color:#cbd5e1;line-height:1.7;margin-top:10px">
      ...
      <b style="color:#10b981">התקפה</b> — שערים ובישולים <b>ביחס לעמדה שלך</b>: כל אחד נמדד מול הממוצע והפיזור של העמדה שלו, לא מול חלוצים. שוערים ומגנים מבקיעים הרבה פחות, ולכן כל שער שלהם קופץ הרבה יותר — שער+בישול למשחק שווה בערך 380 נקודות לשוער, 260 למגן ו-185 לחלוץ.<br>
...
function buildKosher() {
  document.getElementById('kosherFormula').textContent =
    `דירוג כוח = בסיס (רמת הקריירה, כולל חוזק היריבים) + התקפה (שערים ובישולים מול הממוצע והפיזור של אותה עמדה — שער+בישול למשחק שווה ~380 נק׳ לשוער, ~260 למגן, ~185 לחלוץ) + הגנה (כמה ספגת מול מה שהרכב הקבוצה חזה) + כושר 6 חודשים − היעדרות. מוצגים כל השחקנים עם מעל ${KSR_MIN_CAREER_GAMES} משחקים.`;
