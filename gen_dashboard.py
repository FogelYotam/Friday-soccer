...
function profileBody(name, chartId) {
  ...
  const html = `
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid #334155">
      ...
      ${skBadge}
      ${(()=>{const s=EFFECTIVE[name]; return s?`<span title="ציון לקביעת כוחות (בסיס ${s.base}${s.adj>=0?' +':' '}${s.adj} כושר/ELO)${s.est?' · מוערך':''}" style="background:#422006;border:1px solid #a16207;color:#fbbf24;padding:3px 11px;border-radius:12px;font-size:.82rem;font-weight:bold">⭐ ${s.score.toFixed(2)}${s.est?'*':''}</span>`:'';})()}
