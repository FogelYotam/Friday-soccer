...
<div id="tab-kosher" class="tab">
  <details class="card" style="margin-bottom:12px">
    ...
    <div style="font-size:.82rem;color:#cbd5e1;line-height:1.7;margin-top:10px">
      ...
      <b style="color:#10b981">התקפה</b> — שערים ובישולים <b>ביחס לעמדה שלך</b>: כל אחד נמדד מול הממוצע והפיזור של העמדה שלו, לא מול חלוצים. שוערים ומגנים מבקיעים הרבה פחות, ולכן כל שער שלהם קופץ הרבה יותר. <b>שער שווה פי 2 מבישול.</b> מי ששיחק מעט משחקים מקבל ניקוד מכווץ — תפוקה גבוהה ב-20 משחקים פחות אמינה מעשור של נתונים.<br>
...
const ATT_POS_MULT = {'שוער':1.0, 'הגנה':0.6, 'התקפה':1.0};
const ATT_SCALE    = 300;
...
const ATT_SHRINK   = 150;
const attRaw = p => (p.g*GOAL_W + p.a*ASSIST_W) / p.gm;
const ATTACK_PTS = (() => {
  const byRole = {};
  STATS.players.forEach(p => {
    if (!p.gm) return;
    (byRole[roleOf(p.name)] = byRole[roleOf(p.name)] || []).push(attRaw(p));
  });
  const avg = {}, sd = {};
  ...
  STATS.players.forEach(p => {
    ...
    const z = (attRaw(p) - avg[r]) / sd[r];
    const shrink = p.gm / (p.gm + ATT_SHRINK);
...
function buildKosher() {
  document.getElementById('kosherFormula').textContent =
    `דירוג כוח = בסיס (רמת הקריירה, כולל חוזק היריבים) + התקפה (שערים ובישולים מול הממוצע והפיזור של אותה עמדה — שער+בישול למשחק שווה שער פי 2 מבישול, ומכווץ למי ששיחק מעט) + הגנה (כמה ספגת מול מה שהרכב הקבוצה חזה) + כושר 6 חודשים − היעדרות. מוצגים כל השחקנים עם מעל ${KSR_MIN_CAREER_GAMES} משחקים.`;
