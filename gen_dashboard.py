...
def generate():
    ...
    # Manual base ratings (0.75–5.25) for the effective 1–5 score shown in profiles.
    ratings_path = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), 'player_ratings.json')
    try:
        with open(ratings_path, 'r', encoding='utf-8') as f:
            manual_base = json.load(f)
    except FileNotFoundError:
        manual_base = {}
    ratings_json = json.dumps(manual_base, ensure_ascii=False, sort_keys=True)

    html = HTML_TEMPLATE.replace('STATS_PLACEHOLDER', stats_json).replace('GAMES_PLACEHOLDER', games_json).replace('RATINGS_PLACEHOLDER', ratings_json)
...
<nav>
  ...
  <a href="entry.html" style="text-decoration:none"><button>➕ הזנת מחזור</button></a>
</nav>
...
<div id="tab-leaderboard" class="tab">
  ...
  <div class="card" style="margin-top:12px">
    <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;border-bottom:1px solid #334155;padding-bottom:6px;margin-bottom:8px">
      <h3 id="mvpTitle" style="border:none;padding:0;margin:0">🏅 מלכי MVP כל הזמנים</h3>
      <select id="mvpYear" onchange="buildMVPLeaderboard()"></select>
    </div>
...
<div id="tab-kosher" class="tab">
  <details class="card" style="margin-bottom:12px">
    ...
    <div style="font-size:.82rem;color:#cbd5e1;line-height:1.7;margin-top:10px">
      ...
      <b style="color:#3b82f6">בסיס</b> — כמה אתה שווה לפי כל הקריירה שלך. הוא כבר מגלם את <b>חוזק היריבים</b>: ניצחון על קבוצה חזקה הוסיף לך יותר מניצחון על חלשה. העמודה "חוזק יריבים" מראה את הרמה הממוצעת שנגדה שיחקת.<br>
      <b style="color:#10b981">התקפה</b> — שערים ובישולים <b>ביחס לעמדה שלך</b>: כל אחד נמדד מול הממוצע והפיזור של העמדה שלו, לא מול חלוצים. שוערים ומגנים מבקיעים הרבה פחות, ולכן כל שער שלהם קופץ הרבה יותר. <b>שער שווה פי 3.3 מבישול.</b> מי ששיחק מעט משחקים מקבל ניקוד מכווץ — תפוקה גבוהה ב-20 משחקים פחות אמינה מעשור של נתונים.<br>
      <b style="color:#22d3ee">הגנה</b> — כמה הקבוצה שלך ספגה לעומת מה שהרכב הקבוצה חזה. שמרת על שער נקי כשהיו מעט מגנים? בונוס גדול. ספגת הרבה כשהקבוצה הייתה מלאה במגנים? קנס — ולמגנים הקנס כבד יותר.<br>
      <b style="color:#10b981">כושר</b> — כמה עלית או ירדת ב-6 החודשים האחרונים.<br>
      <b style="color:#ef4444">היעדרות</b> — הפחתה על תקופה בלי לשחק: 10 נק׳ לכל חודש. אם <b>היכולת שלך כבר ירדה</b> לפני שנעלמת, ההפחתה גדולה יותר (עד פי 2) — כי זו לא רק חלודה אלא ירידה אמיתית ברמה.<br>
      בטבלה מופיעים כל השחקנים עם מעל 20 משחקים.</p>
    </div>
  </details>
  <div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px">
...
const ELO_MOVES = new Map();   // game object -> {name: {d:delta, r:postRating}}
const ELO_UPSETS = [];         // surprising wins (underdog beat favourite)
const ELO_SERIES = {};         // name -> [{t:timestamp, r:rating}] chronological
const ELO = (() => {
  ...
  [...GAMES].sort((a,b)=>parseDate(a.date)-parseDate(b.date)).forEach(g => {
    ...
    const deltas={};
    tA.forEach(n=>deltas[n]={d:dA}); tB.forEach(n=>deltas[n]={d:-dA});
    ...
    // opp = average rating of the side he played AGAINST (recorded so the table
    // can show that the rating already accounts for opponent strength)
    const bump = (n, d, opp) => {
      R[n]=get(n)+d; peak[n]=Math.max((n in peak)?peak[n]:START, R[n]);
      gm[n]=(gm[n]||0)+1; if(yr){ (hist[n]=hist[n]||{})[yr]=R[n]; }
      (ELO_SERIES[n]=ELO_SERIES[n]||[]).push({t, r:R[n], opp});
      if (deltas[n]) deltas[n].r = Math.round(R[n]);
    };
    tA.forEach(n=>bump(n, dA, Rb)); tB.forEach(n=>bump(n, -dA, Ra));
...
// Power rating = base value (career ELO, which already accounts for the strength
//                of opponents and team-mates in every game)
//              + form movement over the last 6 months
//              − an absence penalty, which bites HARDER when the player was
//                already sliding when he stopped showing up (a drop in ability),
//                and softer for someone who left while holding his level.
const _DAY = 24*60*60*1000;
const ELO_FORM_WINDOW_MONTHS = 6;    // what counts as "recent form"
const ELO_ABSENCE_PER_MONTH  = 10;   // points per month away (after a 1-month grace)
const ELO_ABSENCE_MAX        = 300;
const ELO_DECLINE_GAMES      = 10;   // games before the last one used to detect a slide
function eloForm(name) {
  ...
  // was he declining over his final stretch? amplifies the absence penalty
  const tail = s.slice(-ELO_DECLINE_GAMES);
  const decline = tail.length > 1 ? Math.max(0, tail[0].r - tail[tail.length-1].r) : 0;
  const declineFactor = 1 + Math.min(1, decline/150);   // 1× … 2×

  const months = Math.max(0, daysSince/30 - 1);
  const absence = Math.min(ELO_ABSENCE_MAX,
                           Math.round(months * ELO_ABSENCE_PER_MONTH * declineFactor));

  // average strength of the sides he faced (recent window, else whole career)
  const oppFrom = recentGm ? s.filter(x => x.t >= cutoff) : s;
  const withOpp = oppFrom.filter(x => typeof x.opp === 'number');
  const avgOpp = withOpp.length
    ? Math.round(withOpp.reduce((a,x)=>a+x.opp,0)/withOpp.length) : null;
...
// ── Playing role ─────────────────────────────────────────────────────────────
// Positions are curated by hand — statistics cannot tell a keeper from a
// low-scoring defender, and only the group knows who actually plays where.
...
const ROLE = (() => {
  ...
  const ICON = {'שוער':'🧤', 'התקפה':'⚔️', 'הגנה':'🛡️'};
  const out = {};
  ps.forEach(p => {
    let role, icon, why;
    if (ROLE_OVERRIDE[p.name]) {
      role = ROLE_OVERRIDE[p.name]; why = 'עמדה ידועה';
    } else if (p.gm >= 30 && p.gpg < 0.25 && p.apg < 0.25) {
      role = 'שוער'; why = 'כמעט ללא מעורבות התקפית';
    } else if (p.c >= median) {
      role = 'התקפה'; why = 'תרומה התקפית מעל חציון הקבוצה';
    } else {
      role = 'הגנה'; why = 'תרומה התקפית מתחת לחציון הקבוצה';
    }
    icon = ICON[role] || '';
...
const roleOf = n => (ROLE[n]||{}).role || 'התקפה';
const IS_DEF = n => roleOf(n)==='הגנה' || roleOf(n)==='שוער';

// ── Attacking score, weighted by position ────────────────────────────────────
// A goal or assist from a keeper is worth far more than from a defender, and a
// defender's counts for more than an attacker's. Measured against the average
// for that same position, so everyone is judged against his own job.
// Each player is scored against the average AND the spread of his own position,
// then nudged by a position multiplier. Because keepers vary far less than
// attackers, the same goal moves a keeper much further up his own scale —
// which is why a keeper's goal ends up worth ~2.5× an attacker's.
// ATT_SCALE was fitted so the ranking reproduces the owner's head-to-head calls.
const ATT_POS_MULT = {'שוער':1.0, 'הגנה':0.8, 'התקפה':1.0};
const ATT_SCALE    = 160;
...
const RECENT_ATT = (() => {
  const acc = {};
  GAMES.forEach(g => {
    if (parseDate(g.date) < RECENT_CUTOFF) return;
    [...(g.teamA||[]), ...(g.teamB||[])].forEach(p => {
      if (!p.name) return;
      const a = acc[p.name] = acc[p.name] || {gm:0, g:0, a:0};
      a.gm++; a.g += p.goals||0; a.a += p.assists||0;
    });
  });
  return acc;
})();
const attRaw = name => {
  const a = RECENT_ATT[name];
  return (a && a.gm) ? (a.g*GOAL_W + a.a*ASSIST_W) / a.gm : null;
};
const ATTACK_PTS = (() => {
  const byRole = {};
  STATS.players.forEach(p => {
    const v = attRaw(p.name);
    if (v === null) return;
    (byRole[roleOf(p.name)] = byRole[roleOf(p.name)] || []).push(v);
  });
  const avg = {}, sd = {};
  Object.keys(byRole).forEach(r => {
    const v = byRole[r], m = v.reduce((a,b)=>a+b,0)/v.length;
    avg[r] = m;
    sd[r]  = Math.sqrt(v.reduce((a,b)=>a+(b-m)*(b-m),0)/v.length) || 1;
  });
  const out = {};
  STATS.players.forEach(p => {
    const v = attRaw(p.name);
...
// ── Defensive score: goals conceded vs. what the team's make-up predicted ────
// A side stacked with defenders is expected to concede less. Keeping a clean
// sheet with an attack-heavy team earns more credit; leaking goals with a
// defence-heavy team costs more. Defenders carry more of both.
const DEF_SCALE = 25;
const DEFENSE_PTS = (() => {
  const teamGames = [];
  let totConc=0, totShare=0, n=0;
  GAMES.forEach(g => {
    if (parseDate(g.date) < RECENT_CUTOFF) return;   // recent form only
    const tA=(g.teamA||[]).filter(p=>p.name), tB=(g.teamB||[]).filter(p=>p.name);
    if (!tA.length || !tB.length) return;
    const sA=g.scoreA??0, sB=g.scoreB??0;
    [[tA,sB],[tB,sA]].forEach(([team, conc]) => {
      const share = team.filter(p=>IS_DEF(p.name)).length / team.length;
      teamGames.push({team, conc, share});
      totConc+=conc; totShare+=share; n++;
    });
  });
  if (!n) return {};
  const avgConc = totConc/n, avgShare = totShare/n;
  const acc = {};
  teamGames.forEach(({team, conc, share}) => {
    // a more defensive line-up is expected to concede fewer
    const expected = avgConc * (1 - 0.6*(share - avgShare));
    const diff = expected - conc;                 // + = defended better than expected
    const attackHeavy = Math.max(0, avgShare - share);   // how thin the defence was
    const defenceHeavy = Math.max(0, share - avgShare);
    team.forEach(p => {
      const isDef = IS_DEF(p.name);
      let w;
      if (diff >= 0) w = 1 + attackHeavy*2;                       // credit for holding a thin line
      else           w = isDef ? 1.5 : 1 + defenceHeavy*2;        // blame: defenders always, attackers when the side was stacked back
      if (!acc[p.name]) acc[p.name] = {pts:0, gm:0};
      acc[p.name].pts += diff*w; acc[p.name].gm++;
    });
  });
  const out = {};
...
// Top-3 for an all-time streak record. Name, count and dates sit on one line so
// nothing is pushed to the far edge (which left a big gap in RTL).
function streakTop3(key, unit, color) {
  const known = new Set(STATS.players.map(p=>p.name));
  return Object.entries(STREAKS)
    .filter(([n,s]) => known.has(n) && s[key] > 0)
    .sort((a,b) => b[1][key]-a[1][key]).slice(0,3)
    .map(([n,s],i) => `
      <div style="display:flex;align-items:baseline;gap:5px;font-size:.76rem;margin-bottom:3px">
        ...
        <b style="color:${color};white-space:nowrap">${s[key]} ${unit}</b>
        <span style="font-size:.6rem;color:#64748b;white-space:nowrap">${streakRange(s[key+'From'], s[key+'To'])}</span>
      </div>`).join('');
}
...
function buildOverview() {
  ...
  document.getElementById('heroes').innerHTML = [
    ...
    {icon:'🔥', label:'רצף הניצחונות הארוך בהיסטוריה',
      content: streakTop3('bestW','נצח׳ ברצף','#fbbf24')},
    {icon:'🛡️', label:'הרצף הארוך ביותר ללא הפסד',
      content: streakTop3('bestU','ללא הפסד','#fbbf24')},
    {icon:'🥶', label:'הרצף הארוך ביותר ללא ניצחון',
      content: streakTop3('bestNW','ללא ניצחון','#94a3b8')},
  ].map(h=>`
    <div class="hero-card">
      <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
        <span style="font-size:1.5rem">${h.icon}</span>
  ...
  // Show who is ahead explicitly — the leader's name is green and first, and the
  // score is always written leader-first, so the colour can't be misread.
  document.getElementById('topRivalsOverview').innerHTML = rivals.length ? rivals.map(r => {
    const aW=r.fw, bW=r.t-r.fw-r.d;
    const tied = aW===bW;
    const lead = aW>=bW ? r.p1 : r.p2, trail = aW>=bW ? r.p2 : r.p1;
    const lw = Math.max(aW,bW), tw = Math.min(aW,bW);
    return `<div class="pair-row">
      <span style="flex:1">
        ${tied
          ? `${pl(r.p1)} <span style="color:#fbbf24;font-size:.7rem">תיקו מול</span> ${pl(r.p2)}`
          : `<b style="color:#10b981">${pl(lead)}</b> <span style="color:#475569;font-size:.7rem">מוביל על</span> <span style="color:#94a3b8">${pl(trail)}</span>`}
      </span>
      <span style="white-space:nowrap">
        <span class="chip chip-blue">${r.t} מ׳</span>
        <span class="chip" style="background:#0f172a;color:${tied?'#fbbf24':'#10b981'}">${lw}<span style="color:#475569">:</span><span style="color:#ef4444">${tw}</span></span>
        ${r.d?`<span class="chip" style="background:#0f172a;color:#64748b">${r.d} ת׳</span>`:''}
      </span>
    </div>`;
...
function buildMVPLeaderboard() {
  const sel = document.getElementById('mvpYear');
  if (!sel.options.length) {                       // populate once
    sel.innerHTML = ['<option value="all">כל הזמנים</option>',
      ...[...ALL_YRS].reverse().map(y=>`<option value="${y}">${y}</option>`)].join('');
  }
  const yr = sel.value || 'all';
  document.getElementById('mvpTitle').textContent =
    yr==='all' ? '🏅 מלכי MVP כל הזמנים' : `🏅 מלכי MVP ${yr}`;

  const src = yr==='all'
    ? (STATS.bonus||[])
    : (STATS.bonusByYear||[]).filter(b=>b.yr===yr);
  const rows = [...src].filter(b=>b.mvp>0||b.wg>0).sort((a,b)=>b.mvp-a.mvp||b.wg-a.wg);
  if (!rows.length) { document.getElementById('mvpLeaderboard').innerHTML='<p style="color:#64748b;padding:10px">אין נתונים לשנה זו</p>'; return; }
...
function buildKosher() {
  ...
  _ksrRows = STATS.players
    ...
    .map(({p, f}) => {
      ...
      const def = DEFENSE_PTS[p.name] || 0;
      return {name:p.name, rating:f.rating + att + def, base:f.base, form:f.form,
              absence:f.absence, att, def,
              cur:f.cur, daysSince:f.daysSince, recentGm:f.gm, careerGm:p.gm, trend,
              avgOpp:f.avgOpp, decline:f.decline, declineFactor:f.declineFactor};
    });
  renderKosherTable();
}
  ...
  document.getElementById('kosherTable').innerHTML = `
    ...
    <tbody>${rows.map((p,i)=>{
      ...
      return `<tr>
        ...
        <td style="color:#94a3b8">${p.avgOpp ?? '-'}</td>
        <td>${signed(p.att)}</td>
        <td>${signed(p.def)}</td>
        <td>${signed(p.form)}</td>
        <td style="text-align:center">${ti(p.trend)}</td>
        <td style="color:#64748b">${p.recentGm||'-'}</td>
        <td>${p.absence ? `<span style="color:#ef4444">-${p.absence}</span><div style="font-size:.58rem;color:#475569">${away(p)}${p.declineFactor>1.05?` · ירידה ×${r2(p.declineFactor)}`:''}</div>` : '<span style="color:#475569">—</span>'}</td>
        <td>${skHtml(p.name)}</td>
        <td>
...
function profileBody(name, chartId) {
  ...
  const html = `
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid #334155">
      ...
      ${skBadge}
      ${(()=>{const s=EFFECTIVE[name]; return s?`<span title="ציון לקביעת כוחות (בסיס ${s.base}${s.adj>=0?' +':' '}${s.adj} כושר/ELO)${s.est?' · מוערך':''}" style="background:#422006;border:1px solid #a16207;color:#fbbf24;padding:3px 11px;border-radius:12px;font-size:.82rem;font-weight:bold">⭐ ${s.score>5?'5+':s.score.toFixed(2)}${s.est?'*':''}</span>`:'';})()}
      ${(()=>{const e=ELO[name]; return e?`<span title="דירוג ELO · שיא ${e.peak}" style="background:#0f172a;border:1px solid #334155;color:${e.rating>=1600?'#10b981':e.rating>=1500?'#3b82f6':'#f59e0b'};padding:3px 11px;border-radius:12px;font-size:.82rem;font-weight:bold">ELO ${e.rating}</span>`:'';})()}
    ...
    ${yrs.length>1 ? `
    <div class="card" style="padding:10px;margin-bottom:12px">
      <div style="font-size:.76rem;color:#64748b;margin-bottom:4px">📈 מגמה לפי שנה <span style="font-weight:normal;color:#475569">· <span style="color:#60a5fa">עמודה</span> = משחקים · <span style="color:#fbbf24">עיגול</span> = % ניצחון</span></div>
...
function makeTrendChart(chartId, yrs) {
  ...
  // games per year → bars; win% → circles (own 0-100 axis)
  return new Chart(ctx, {
    data:{
      labels: yrs.map(y=>y.yr),
      datasets:[
        {type:'bar', label:'משחקים', data:yrs.map(y=>y.gm),
          backgroundColor:'rgba(59,130,246,.45)', borderRadius:3, yAxisID:'y2', order:2},
        {type:'line', label:'% ניצחון', data:winPct, yAxisID:'y',
          showLine:false, pointRadius:6, pointHoverRadius:8,
          pointBackgroundColor:'#fbbf24', pointBorderColor:'#1e293b', pointBorderWidth:2, order:1},
      ]
    },
    options:{
      plugins:{
        ...
        tooltip:{callbacks:{
          ...
          label:c=> c.dataset.type==='bar' ? `${c.raw} משחקים` : `${c.raw}% ניצחון`
        }}
      },
      scales:{
