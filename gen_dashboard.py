...
<div id="tab-kosher" class="tab">
  <details class="card" style="margin-bottom:12px">
    ...
    <div style="font-size:.82rem;color:#cbd5e1;line-height:1.7;margin-top:10px">
      ...
      <b style="color:#10b981">התקפה</b> — שערים ובישולים <b>ביחס לעמדה שלך</b>: כל אחד נמדד מול הממוצע והפיזור של העמדה שלו, לא מול חלוצים. שוערים ומגנים מבקיעים הרבה פחות, ולכן כל שער שלהם קופץ הרבה יותר — שער+בישול למשחק שווה בערך 400 נקודות לשוער, 330 למגן ו-160 לחלוץ.<br>
...
const ATT_POS_MULT = {'שוער':1.5, 'הגנה':1.2, 'התקפה':1.0};
const ATT_SCALE    = 110;
const ATTACK_PTS = (() => {
  const byRole = {};
  STATS.players.forEach(p => {
    if (!p.gm) return;
    (byRole[roleOf(p.name)] = byRole[roleOf(p.name)] || []).push((p.g+p.a)/p.gm);
  });
  const avg = {}, sd = {};
  ...
  STATS.players.forEach(p => {
    ...
    const z = ((p.g+p.a)/p.gm - avg[r]) / sd[r];
...
function buildKosher() {
  document.getElementById('kosherFormula').textContent =
    `דירוג כוח = בסיס (רמת הקריירה, כולל חוזק היריבים) + התקפה (שערים ובישולים מול הממוצע והפיזור של אותה עמדה — שער+בישול למשחק שווה ~400 נק׳ לשוער, ~330 למגן, ~160 לחלוץ) + הגנה (כמה ספגת מול מה שהרכב הקבוצה חזה) + כושר 6 חודשים − היעדרות. מוצגים כל השחקנים עם מעל ${KSR_MIN_CAREER_GAMES} משחקים.`;
