...
function buildKosher() {
  document.getElementById('kosherFormula').textContent =
    `דירוג כוח = בסיס (רמת הקריירה — כבר מגלמת את חוזק היריבים והשותפים בכל משחק) + כושר מ-6 החודשים האחרונים − היעדרות (${ELO_ABSENCE_PER_MONTH} נק׳ לחודש, ועד פי 2 אם היכולת כבר ירדה לפני ההיעלמות; מקסימום ${ELO_ABSENCE_MAX}). מוצגים כל השחקנים עם מעל ${KSR_MIN_CAREER_GAMES} משחקים.`;
