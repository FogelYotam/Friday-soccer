...
const ROLE_OVERRIDE = {
  ...
  // ⚔️ התקפה
  "מוטי": "התקפה", "לירן": "התקפה", "שי": "התקפה", "זיו": "התקפה", "יונתן": "התקפה",
  // 🛡️ הגנה
...
function gameSummaryText(game) {
  ...
  const eloLine = team => (team||[]).map(p => {
    const m = moves[p.name];
    if (!m) return `  ${p.name}`;
    const sign = m.d>=0 ? '+' : '';
    return `  ${p.name}: ${m.r} (${sign}${Math.round(m.d)})`;
  }).join('\n');
