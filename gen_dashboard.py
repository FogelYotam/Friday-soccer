...
<div id="tab-kosher" class="tab">
  <details class="card" style="margin-bottom:12px">
    ...
    <div style="font-size:.82rem;color:#cbd5e1;line-height:1.7;margin-top:10px">
      ...
      <b style="color:#10b981">התקפה</b> — שערים ובישולים, אבל <b>ביחס לעמדה שלך</b>: שער של שוער שווה פי 4 משל חלוץ, ושל מגן פי 2. כל אחד נמדד מול הממוצע של העמדה שלו, לא מול חלוצים.<br>
...
const ATT_WEIGHT = {'שוער':4, 'הגנה':2, 'התקפה':1};
const ATT_SCALE  = 25;
const ATTACK_PTS = (() => {
  const byRole = {};
  STATS.players.forEach(p => {
    ...
    (byRole[r] = byRole[r] || []).push((p.g+p.a)/p.gm);
  });
  const avg = {};
  Object.keys(byRole).forEach(r => avg[r] = byRole[r].reduce((a,b)=>a+b,0)/byRole[r].length);
  const out = {};
  STATS.players.forEach(p => {
    if (!p.gm) return;
    const r = roleOf(p.name);
    const rel = (p.g+p.a)/p.gm - (avg[r]||0);
    out[p.name] = Math.round(rel * (ATT_WEIGHT[r]||1) * ATT_SCALE);
  });
  return out;
})();
...
function buildKosher() {
  document.getElementById('kosherFormula').textContent =
    `דירוג כוח = בסיס (רמת הקריירה, כולל חוזק היריבים) + התקפה (שערים ובישולים ביחס לעמדה — של שוער שווה ×${ATT_WEIGHT['שוער']}, של מגן ×${ATT_WEIGHT['הגנה']}, של חלוץ ×${ATT_WEIGHT['התקפה']}) + הגנה (כמה ספגת מול מה שהרכב הקבוצה חזה) + כושר 6 חודשים − היעדרות. מוצגים כל השחקנים עם מעל ${KSR_MIN_CAREER_GAMES} משחקים.`;
