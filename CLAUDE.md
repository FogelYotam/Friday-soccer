...
- **דירוג כוח** (`eloForm`) = **בסיס + כושר − היעדרות**:
  ...
  - זכאות: מעל `KSR_MIN_CAREER_GAMES` (20) משחקי קריירה **וגם** משחק אחד לפחות ב-`KSR_MAX_INACTIVE_DAYS` (730 = שנתיים). מי שנעדר יותר יורד מהטבלה אוטומטית — הדירוג שלו נשמר והוא חוזר ברגע שישחק.
