...
- **דירוג כוח** (`eloForm`) = **בסיס + כושר − היעדרות**:
  ...
  - *היעדרות* — `ELO_ABSENCE_PER_MONTH` (10) נק׳ לחודש אחרי חודש חסד, עד `ELO_ABSENCE_MAX` (300). מוכפל ב-`declineFactor` (1×–2×) אם היכולת ירדה ב-`ELO_DECLINE_GAMES` (10) המשחקים האחרונים לפני ההיעלמות.
  - *חוזק יריבים* — כל רשומה ב-`ELO_SERIES` שומרת `opp` (ממוצע ה-ELO של הקבוצה היריבה), ומוצג בטבלה כעמודה.
  - זכאות: `KSR_MIN_CAREER_GAMES` (10) משחקי קריירה ומעלה **וגם** משחק אחד לפחות ב-`KSR_MAX_INACTIVE_DAYS` (730 = שנתיים). מי שנעדר יותר יורד מהטבלה אוטומטית — הדירוג שלו נשמר והוא חוזר ברגע שישחק.
- **התקפה/הגנה בדירוג** — `ATTACK_PTS` (שערים ובישולים מול ממוצע *העמדה*, `ASSIST_W`=0.3 כך ששער שווה פי ~3.3) ו-`DEFENSE_PTS` (ספיגות מול מה שהרכב הקבוצה חזה). הפרמטרים כוילו מול ~30 השוואות זוגות שהבעלים סיפק (27/29 מתקיימים).
- **`ROLE_OVERRIDE`** — מפת עמדות ידנית (שוער/הגנה/התקפה). לערוך שם כדי לתקן עמדה של שחקן.
...
- **`MIN_GAMES_THRESHOLD = 10`** — פחות מ-10 משחקים = נחשב אורח, לא מופיע בסטטיסטיקות (42 שחקנים עומדים בסף)
